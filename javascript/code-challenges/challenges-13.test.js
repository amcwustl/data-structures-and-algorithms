...
const unenrollBrook = (arr) => {
  let newArr =[];
  for (let i of arr){
    if (i.filter(name => !name.includes('Brook')).length > 0){
      newArr.push(i.filter(name => !name.includes('Brook')));
    } else {
      newArr.push([]);
    }
  }
