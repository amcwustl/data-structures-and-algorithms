...
const getStatName = (arr, minBaseStat) => {
  let newArr = [];
  let objArr = arr.filter(element => element.baseStat > minBaseStat);
  for (let i in objArr){
    newArr.push(objArr[i].stat.name);
  }
  return newArr;
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(element => !('children' in element));
};
...
const evenOddNumericValues = (arr) => {
  let filtArr = arr.filter(element => typeof element === 'number');
  let newArr = filtArr.map(x => {
    if (x%2 === 0){
      return 'even';
    } else {
      return 'odd';
