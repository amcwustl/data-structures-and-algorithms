...
const stepActions = (recipe) => {
  ...
  for (let i in recipe.steps){
    result.push(recipe.steps[i].slice(0,recipe.steps[i].indexOf(' ')));
  }
  return result;
};
...
const totalSumCSV = (str) => {
  let total = 0;
  let arr = str.split(',');
  total = arr.reduce((acc,curVal) => {
    acc= acc + Number(curVal);
    return acc;
  },0);
  return total;
};
...
const removeVowels = (str) => {
  ...
  let newstr = str.split('').filter((char) => !vowels.includes(char)).join('');
  return newstr;
...
const extractVowels = (str) => {
  let vowels = ['a','e','i','o','u','A','E','I','O','U'];
  let newstr = str.split('').filter((char) => !vowels.includes(char)).join('');
  let vowstr = str.split('').filter((char) => vowels.includes(char)).sort().join('');
  return [newstr, vowstr];
};
