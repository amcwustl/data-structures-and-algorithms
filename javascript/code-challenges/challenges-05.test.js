...
const toLastNames = people => {
  return people.reduce((acc, curVal) => {
    acc.push(curVal.firstName + ' ' + curVal.lastName);
    return acc;
  },[]);
};
...
const addValues = (arr) => {
  return arr.reduce((acc, curVal) => acc = acc + curVal,0);
};
...
const calculateAverage = (arr) => {
  let final = arr.reduce((acc, curVal) => {
    acc.count = acc.count + 1;
    acc.sum = acc.sum + curVal;
    return acc;
  },{ count: 0, sum: 0 });
  return final.sum / final.count;
};
...
const extractStats = (snorlaxData) => {
  return snorlaxData.stats.reduce((acc, curVal) => {
    let name = curVal.stat.name;
    let base = curVal.baseStat;
    acc[name] = base;
    return acc;
  },{});
};
...
const extractChildren = (arr) => {
  let chars =  arr.filter((element) => {
    return element.name.toLowerCase().includes('a');
  });
  return chars.reduce((acc, curVal) => {
    if (curVal.children) {
      return acc.concat(curVal.children);
    }
    return acc;
  }, []);
};
