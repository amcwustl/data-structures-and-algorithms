...
const count = (target, input) => {
  return input.map(element => element.filter(value => value === target)).reduce((acc, curVal) => acc + curVal.length,0);
};
...
const totalSum = (input) => {
  return input.map(element => element.reduce((acc, curVal) => acc += curVal,0)).reduce((acc,curVal) => acc += curVal,0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(element => element.filter(value => value % 5 === 0 && typeof(value) === 'number' )).map(element => element.map(value => 2 ** value));
};
...
let findShortest = (data) => {
  let min = Infinity;
  let newMin = data.reduce((acc, curVal) => {
    if (Number(curVal.height) < acc){
      acc = Number(curVal.height);
      return acc;
    } else {
      return acc;
    }
  },min);
  return data.filter(object => Number(object.height) === newMin)[0].name;
};
