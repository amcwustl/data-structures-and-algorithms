...
const screenForNames = (arr) => {
  const regex = /^(?:Mr\.|Mrs\.|Ms\.|Dr\.)\s[A-Za-z]+$/gm;
  let newArr=[];
  for (let i of arr){
    if (regex.test(i)){
      newArr.push(i);
...
const detectTicTacToeWin = (board) => {
  let regex = /^(XXX|OOO)$/g;
  for (let i of board){
    if(regex.test(i[0]+i[1]+i[2])){
      return true;
    }
  }
  for (let j in board){
    if (regex.test(board[0][j] + board[1][j] + board[2][j])){
      return true;
    }
  }
  if (regex.test(board[0][0]+ board[1][1] + board[2][2])||regex.test(board[0][2]+board[1][1]+board[2][0])){
    return true;
  }
  return false;
};
