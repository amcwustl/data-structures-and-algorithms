...
const totalCharacters = (arr) => {
  ...
  for (let i in arr){
    ...
    if (arr[i].children){
      total = total + arr[i].children.length
    }
  }
  return total;
};
...
const houseSize = (arr) => {
  ...
  for (let i in arr){
    sizes.push({house: arr[i].house, members: arr[i].members});
  }
...
const houseSurvivors = (arr) => {
  ...
  for (let i in arr) {
    ...
    if (arr[i].spouse && !deceasedSpouses.includes(arr[i].spouse)){
      total ++;
    }
    if (arr[i].children){
      total = total + arr[i].children.length;
  ...
  for (let i in arr){
    survivors.push({house: arr[i].house, members: arr[i].members});
  }
